...
### 摄入内容
- 文件：topics/timescales/papers/kleppner2006.yaml
- 贡献类型：framework
- 核心贡献：指出当原子钟精度进入 10⁻¹⁶–10⁻¹⁸ 水平后，引力红移（~1×10⁻¹⁶/m）使全球分布式时钟比较失去物理意义——引力改变的是时间本身而非扰动时钟。分析了大地水准面不确定度（30–50 cm → ~3×10⁻¹⁷）和固体潮（20 cm → ~2×10⁻¹⁷）对时间比对的终极限制。总结 UTC 闰秒机制历史（截至 2006 年 23 次）。
- 新增节点：2 实体 + 3 原理（含 2 foundational）+ 1 方法 + 4 指标 + 6 关系
