...
> 状态：`[ ]` 待处理 | `[→]` 处理中 | `[x]` 已完成
> 
> 更新规则：完成一篇后将 `[ ]` 改为 `[x]`，同时在 SCHEMA.md 的"已处理论文"表中补充。

## 处理优先级说明

**高优先级**（核心方法/原理论文）：Matei 2017 (mHz 线宽)、Kessler 2012 (Si 单晶腔)、Cole (晶体涂层)、Webster (防振腔)、Legero (热膨胀调谐)

**中优先级**（应用/改进论文）：各组的实验实现论文

**低优先级**（基础/外围论文）：Kogelnik 1966 (高斯光束理论)、Winful 1985 (光纤双折射)、应用类

---

## Zotero Collection: 超稳腔稳频激光

### 核心论文

- [x] `694DPR5F` | Drever 1983 | Laser phase and frequency stabilization using an optical resonator → `drever1983.yaml`
...
处理每篇论文时，参照 SCHEMA.md 中的模板提取节点和关系，生成对应的 `papers/{author}{year}.yaml` 文件。
...
curl -s -H "Host: 127.0.0.1:23119" \
  ...
  | python3 -c "import json,sys; [print(i['data'].get('path','') or i['data'].get('filename','')) for i in json.load(sys.stdin) if i['data'].get('itemType')=='attachment']"
# PDF 在 WSL2 中的路径格式：/mnt/d/Users/hjian/Zotero/storage/{STORAGE_KEY}/{filename}.pdf
```

### 自动化处理脚本提示

可在此仓库目录下运行 Claude Code，用 `/loop` 命令自动处理队列：
```
cd /home/hjian/sci-logic-kb
claude  # 然后在 Claude 中说：处理 QUEUE.md 中下一篇待处理论文
```
