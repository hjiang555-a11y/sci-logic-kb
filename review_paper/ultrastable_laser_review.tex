...
Combining silicon substrates at 4–17 K with crystalline coatings has pushed instability to the $4\times10^{-17}$ level \cite{chen2025,lee2026} and linewidths below 10 mHz \cite{matei2017}. These systems now operate in closed‑cycle cryostats, making them practical for long‑term clock operation.

\subsection{Fiber‑Based References (2010s–Present)}
As an alternative to rigid cavities, fiber‑delay‑line (FDL) stabilization uses the round‑trip time of light in a long optical fiber as a frequency reference \cite{jiang2010}. Although limited by fiber thermal noise (${\sim}10^{-15}$ at 1 s), FDL systems are compact, vibration‑insensitive, and suitable for field deployment. Recent advances with hollow‑core fibers \cite{belardi2015} and recirculating interferometers \cite{gao2025} have improved performance.
