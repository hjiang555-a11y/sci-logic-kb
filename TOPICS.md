...
│   └── 稳频策略（多级级联、多腔平均）
│
├── 2. 光学频率梳 (Optical Frequency Combs)      ← 已建，8篇论文（6框架综述+1框架+1技术）
│   ├── 锁模激光器（钛宝石、光纤、固体）
│   ├── 微腔光梳 (Microresonator Combs / Dissipative Kerr Solitons)
│   ├── 电光调制梳 (EO Combs)
│   ├── 中红外频率梳 (Mid-IR Combs)
│   ├── 梳齿稳定（f-2f 自参考、光学锁定）
...
│   └── 微波-光学比较
│
├── 5. 时间频率传递 (Time & Frequency Transfer)   ← 待建（统一光频传递与微波传递）
│   ├── 光纤链路传递（光频 / 微波 / 授时）
│   ├── 自由空间与卫星链路
│   ├── 双向卫星时间频率传递 (TWSTFT)
│   ├── GNSS 载波相位 / PPP
...
> 当前已入库的 3 个下游专题中，`giunta2019.yaml`、`fortier2026.yaml`、`dimarcq2024.yaml` 的主要作用都是定义专题顶层架构和跨专题接口；其中光学频率梳专题已进一步扩展到多篇框架综述与技术论文，不应与“仅有框架骨架”的专题混淆。

### 第一梯队（与超稳激光耦合最紧密）

1. **光学频率梳**：超稳激光的直接下游；梳齿相位噪声直接影响光钟性能。已建立完整框架（8篇论文），覆盖：锁模激光器梳、微腔光梳/DKS、中红外梳、双梳光谱学、频率梳光谱学。下一步重点：填充具体技术论文（如新型微腔平台、电光梳实验）。
2. **光钟**：超稳激光的主要应用场景；询问激光线宽是当前限制之一

### 第二梯队

3. **时间频率传递**：统一光频传递、微波传递与授时链路，是远程光钟比对和跨实验室溯源的必要环节
...
│   └── papers/
├── time-frequency-transfer/     # 专题5（待建，统一光频传递与微波传递）
│   └── papers/
