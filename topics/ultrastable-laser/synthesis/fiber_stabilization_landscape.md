...
| σ_y 台阶 | 年份 | 使能配置 | 代表文件 |
|---------|------|---------|---------|
| ~10⁻¹⁴ @ 0.1–1 s (首次) | 2009–2010 | 多模光纤延迟线 + AOM 外差 | Kéfélian 2009, Jiang 2010 |
| ~7×10⁻¹⁵ @ 0.1–1 s | 2015 | 500 m SMF + 双 AOM + 温控 | Dong 2015 |
| **3.2×10⁻¹⁵ @ 1 s**（短期）/ 1.1×10⁻¹⁴ @ 1000 s（长期） | 2023 | 全光纤 + 多层热屏蔽 | Huang 2023 |
| **6.3×10⁻¹⁵ @ 16 ms** 🏆 | 2025 | 自零差光纤，4–200 Hz 热噪声极限 | [jeon2025](../papers/jeon2025.yaml) |

**光纤子分支 SOTA**：
- **短期（τ ≤ 1 s）**：Huang 2023 的 3.2×10⁻¹⁵ @ 1 s 是 **1 秒积分点**已报道最低值；Jeon 2025 的 6.3×10⁻¹⁵ @ 16 ms 是**更短时标**（非 1 s）的最低值
- **长期（τ ≥ 1000 s）**：Huang 2023 的 1.1×10⁻¹⁴ @ 1000 s
- 与 FP 腔分支 SOTA（Lee 2026 mod σ_y = 2.5×10⁻¹⁷ @ 10 s）差距 **~10³×** @ 1 s 等效比较
...
| 条目 | 原页面声明 | YAML 实测 | 结论 |
|------|----------|---------|------|
| Jiang 2010 / Kefelian 2009 σ_y | "~10⁻¹⁴" | Jiang: 10⁻¹⁴ @ 0.1–1 s；Kefelian 无 σ_y（线宽 8 Hz） | ✅ 一致（时标 0.1–1 s 已补） |
| Dong 2015 σ_y | "~7×10⁻¹⁵" | 7×10⁻¹⁵ @ 0.1–1 s | ✅ |
| Huang 2023 σ_y | "~1.1×10⁻¹⁴ 长期" | **3.2×10⁻¹⁵ @ 1 s** + 1.1×10⁻¹⁴ @ 1000 s | **已修正**（原表格漏写短期 3.2×10⁻¹⁵ @ 1 s —— 这其实是光纤子分支 τ=1 s 当前最佳！） |
| Jeon 2025 σ_y | "6.3×10⁻¹⁵" | 6.3×10⁻¹⁵ **@ 16 ms**（非 1 s） | **已修正**（τ 限定词必须） |
| 与 FP 差距 | "~250×" | 若以 τ=1 s 等效比较（Huang 2023 的 3.2e-15 vs Lee 2026 的 ~2.5e-17 @ 10s）则为 ~10³× | **已修正**（原 250× 是拿 Jeon @ 16ms 对 Lee @ 10s 比，时标错配）|
| 光纤子分支 SOTA 归属 | 全部给 Jeon 2025 | 应**分时标**：τ=1s Huang 2023、τ=16ms Jeon 2025、长期 Huang 2023 | **已修正** |

**遗留动作**：
- [ ] 专家确认"子分支 SOTA 按时标分桶"的呈现是否符合专题策展风格
- [ ] 考虑补 Huang 2023 的 met.fractional_freq_instability YAML 节点（当前用 met.* 节点引用但 σ_y @ 1s 数据应显式化）
