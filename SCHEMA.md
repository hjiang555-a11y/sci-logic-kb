...
- **当前已建专题**：超稳激光（`topics/ultrastable-laser/`，78 篇论文）、光学频率梳（`topics/optical-frequency-combs/`，101 篇论文）
...
├── 专题2：光学频率梳 ← 已建（topics/optical-frequency-combs/，101 篇，三层应用‑技术‑原理架构）
...
1. **光学频率梳**：超稳激光的直接下游；梳齿相位噪声直接影响频率标准性能。
   - **当前状态**：已建立应用‑技术‑原理三层完整框架，累计 **101 篇论文**
   - **主要缺口**：尚无 `synthesis/` 页，读者入口与跨论文诊断能力明显落后于超稳激光专题
   - **下一步重点**：按 9 条子域主线补首批综合页（频率综合 / 双梳光谱 / 微梳平台 / 光谱应用等）
2. **超稳激光**：78 篇论文的主线整治已基本闭环，是当前最成熟专题。
   - **当前状态**：breakthrough chain-gap / orphan 已清零，breakthrough-only reasoning closure 达 100%
   - **主要缺口**：8 个 synthesis 页面 freshness 待收口，仍有少量专家签字 / 复用整治尾项

### 第二梯队（骨架已激活）

3. **频率标准**：当前 1 篇 framework 论文，负责定义顶层架构与跨专题接口；后续以代表性物种/微波标准论文扩容。
4. **时间标尺与钟组**：当前 1 篇 framework 论文；后续优先补 UTC/TAI、本地 UTC(k)、钟组合成算法。

### 第三梯队（架构占位）

5. **时间频率传递**：当前 0 篇，仅保留架构位；待代表性光纤/卫星/GNSS 论文后再激活 Level 1 实体。
6. **时频计量数学基础**：暂以 `topics/shared/` 的共享模块方式演进，待内容积累后再决定是否独立成专题。
