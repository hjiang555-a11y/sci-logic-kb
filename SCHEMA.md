...
- **已建专题**（维护中 / 已入库）：超稳激光（`topics/ultrastable-laser/`）、光学频率梳（`topics/optical-frequency-combs/`）
- **成长 / 初建专题**：频率标准（`topics/frequency-standards/`）、时间频率传递（`topics/time-frequency-transfer/`）、时频计量数学基础（`topics/shared/`）、时间标尺与钟组（`topics/timescales/`）
- **专题体系与建设阶段**：详见 [`TOPICS.md`](TOPICS.md)；**精确论文数 / 节点数**以 `python scripts/stats.py` 或 [`INDEX.md`](INDEX.md) 为准，本文件不维护硬编码计数
...
├── 专题2：光学频率梳 ← 已建（topics/optical-frequency-combs/，三层应用‑技术‑原理架构）
...
1. **光学频率梳**：超稳激光的直接下游；梳齿相位噪声直接影响频率标准性能。
   - **当前状态**：已建立应用‑技术‑原理三层完整框架，论文规模成熟（精确计数见 [`INDEX.md`](INDEX.md)）
   - **主要缺口**：`synthesis/` 覆盖仍少于超稳激光，读者入口与跨论文诊断能力仍在补齐
   - **下一步重点**：按 9 条子域主线补首批综合页（频率综合 / 双梳光谱 / 微梳平台 / 光谱应用等）
2. **超稳激光**：主线整治已基本闭环，是当前最成熟专题。
   - **当前状态**：breakthrough chain-gap / orphan 已清零，breakthrough-only reasoning closure 达 100%
   - **主要缺口**：synthesis 页面 freshness 待收口，仍有少量专家签字 / 复用整治尾项

### 第二梯队（骨架已激活）

3. **频率标准**：以光钟 framework + 代表性物种论文扩容中，下一步重点是微波标准与跨物种比对。
4. **时间频率传递**：evidence 批量入库完成，下一步是 breakthrough 档审核与架构回填。
5. **时间标尺与钟组**：仍为 framework 骨架；待 UTC/TAI、本地 UTC(k)、钟组合成算法代表论文。

### 第三梯队（共享模块）

6. **时频计量数学基础**：以 `topics/shared/` 的共享模块方式演进，待内容积累后再决定是否独立成专题。

### 当前重点：从“单专题整治”转入“覆盖扩展 + 综合页维护”
为提升知识库的科研推理能力，当前重点任务是：

1. **补覆盖**：为已成规模但缺少读者入口的专题补 `synthesis/` 综述层
2. **保新鲜**：对成熟专题的综合页执行 freshness 维护，避免 YAML 与综述层脱节
3. **控复用**：提升跨文件/跨专题复用率，减少重复定义
4. **稳骨架**：为下游专题持续补代表论文，但不提前投入大规模整治
...
4. **现有内容不破坏**：已入库专题的 YAML 内容不做破坏性修改，仅调整存放路径
...
| 光学频率梳 | 101 | 7 | 94 | 全部 ✅ v4.1–v4.5 | 2026-04-23 |
