...
> **版本**：v5.0（2026-05-09，当前版本）
> **v5.0 状态**：v5.0 三层原子推理架构（Evidence/Logic/Consensus）已启动，Phase 0-1 完成。
>   v4.5 的事实层（topics/*/papers/*.yaml）保持为唯一真源，v5.0 作为推理叠加层在其上运行。
>
> **v5.0 变更摘要**：
> 1. **新增三层推理叠加层**（§十一）— Evidence层（自动证据索引）/ Logic层（人工推理链）/ Consensus层（领域共识报告）
> 2. **新增脚本**：`build_evidence_index.py`（证据提取）、`validate_chains.py`（推理链验证）、`reasoning_stats.py`（推理就绪度量）
> 3. **新增目录**：`evidence/`（证据注册表）、`logic/chains/`（推理链）、`consensus/`（共识报告）
> 4. **v4.5 所有规范不变**——YAML模板、节点类型、关系谓词、贡献分级均保持向后兼容
>
> **v4.5 变更摘要（上版）**：
...
- **当前专题**（6 个，精确计数见 [`INDEX.md`](INDEX.md) 或运行 `python scripts/stats.py`）：
  - **超稳激光**（`topics/ultrastable-laser/`）— 维护中
  - **光学频率梳**（`topics/optical-frequency-combs/`）— 已入库
  - **频率标准**（`topics/frequency-standards/`）— 成长中（光学+微波原子钟）
  - **时间频率传递**（`topics/time-frequency-transfer/`）— 成长中
  - **时间标尺**（`topics/timescales/`）— 初建
  - **共享基础**（`topics/shared/`）— 跨专题数学基础与公共节点
...
> **定位**: v5.0 是叠加在 v4.5 事实层之上的推理层。v4.5 的 YAML 节点图保持为唯一真源，v5.0 提供跨论文的因果推理、证据聚合和共识建立。
...
v5.0 推理叠加层（新增）
═══════════════════════════════════
  Consensus（共识层）── consensus/*.yaml
    关键指标的最佳值、时间线、争论状态

  Logic（逻辑层）── logic/chains/*.yaml
    因果推理链：问题→限制原理→条件变量→突破路径→当前状态

  Evidence（证据层）── evidence/registry_*.yaml
    从 v4.5 BOUNDED-BY 关系自动提取的原子证据索引
═══════════════════════════════════
v4.5 事实层（不变）
...
| 层 | 如何生成 | 如何验证 | 更新频率 |
|----|---------|---------|---------|
| Evidence | `build_evidence_index.py` 自动扫描 v4.5 YAML | validate_chains.py 检查引用一致性 | 每次 v4.5 变更后 |
| Logic | 人工/AI 协同编写 | validate_chains.py 验证所有 evidence 引用 | 当新论文改变推理结构时 |
| Consensus | 人工/AI 协同编写 | 人工审核 | 当新论文刷新指标记录时 |

### 11.3 Evidence 层规范

证据注册表由 `scripts/build_evidence_index.py --topic <topic>` 自动生成，不可手工编辑。

```yaml
# evidence/registry_<topic>.yaml
topic: ultrastable-laser
generated: 2026-05-09 00:06 UTC
schema_version: v5.0-evidence
summary:
  total_evidence_units: 55
  unique_limiting_principles: 14
evidence_groups:
  - limiting_principle: pri.brownian_thermal_noise_fdt
    supporting_papers: 32
    year_range: 1999-2026
    best_evidence: {paper, author, year, claim, is_system_limit, limit_status}
    breakthrough_paths: [...]
    evidence_units:
      - {relation_id, paper, author, year, is_system_limit, limit_status, verification, claim}
```
...
```yaml
# logic/chains/<chain_id>.yaml
chain_id: sigma_y_cavity_stability
question: "What determines the ultimate σ_y(τ≈1s) of FP cavity-stabilized lasers?"
domain: ultrastable-laser
confidence: draft | established
last_updated: YYYY-MM-DD

current_best: {value, system, paper}

limiting_principle:
  id: pri.brownian_thermal_noise_fdt  # 必须引用 v4.5 中已定义的 pri.*
  name: "..."
  statement: "..."
  defined_by: <yaml_file>.yaml

condition_variables:
  - {symbol, name, effect, current_best, bottleneck}
...
evidence:
  - {relation_id: rel.xxx, claim: "..."}  # relation_id 必须存在于 v4.5
```

**关键约束**:
- `limiting_principle.id` 必须以 `pri.` 开头（引用 v4.5 中已定义的原理节点）
- `evidence[].relation_id` 必须可追溯到 v4.5 YAML 中的实际 relation ID
- `edges[].from/to` 引用链内节点或 v4.5 的 pri.* 节点
...
```yaml
# consensus/<metric_safe_name>.yaml
...
consensus:
  best_demonstrated_value: "..."
  best_demonstrated_system: "..."
  theoretical_limit: "..."
  remaining_gap: "..."

timeline:
  - {year, value, system, significance, paper}
```

### 11.6 推理就绪度量

运行 `scripts/reasoning_stats.py` 查看 v5.0 度量：

| 度量 | 含义 |
|------|------|
| Chain Coverage | 每个专题的推理链条数 |
| Evidence Registries | 每个专题的证据注册表规模 |
| Chain-to-evidence link density | 推理链引用的证据占总证据的比例 |
| Breakthrough path ratio | 有突破路径的 BOUNDED-BY 占比 |
| Consensus coverage | 有共识报告的专题数 |

### 11.7 v5.0 目录结构

```
sci-logic-kb/
├── evidence/                    # 证据注册表（脚本自动生成）
│   └── registry_<topic>.yaml
├── logic/chains/                # 推理链（人工编写）
│   └── <chain_id>.yaml
├── consensus/                   # 共识报告（人工编写）
│   └── <metric>.yaml
├── scripts/
│   ├── build_evidence_index.py  # v4.5 → evidence 提取
│   ├── validate_chains.py       # 推理链引用验证
│   ├── generate_consensus.py    # 共识报告辅助生成
│   └── reasoning_stats.py       # v5.0 推理就绪度量
└── docs/v5/PLAN.md              # v5.0 执行计划
```
